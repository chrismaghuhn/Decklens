/**
 * Test helpers for bot-ml tests.
 * Re-exports bot-core helpers plus ML-specific utilities.
 */

import type { GameState, Card, Permanent, PlayerState, GameAction } from '@mtg/game-engine';
import {
  createSimpleCard,
  createPlayerState,
...
export function makeCreature(
  ...
  return createSimpleCard(name, 'Creature — Test', cost, owner, {
    power, toughness, colors: [], colorIdentity: [], ...opts,
  });
}

export function makeInstant(name: string, cost: string, owner: 0 | 1, opts?: Partial<Card>): Card {
  return createSimpleCard(name, 'Instant', cost, owner, {
    colors: [], colorIdentity: [], ...opts,
  });
}

export function makeLand(name: string, owner: 0 | 1, opts?: Partial<Card>): Card {
  return createSimpleCard(name, 'Land — Forest', '', owner, {
    oracleText: '{T}: Add {G}.', colors: [], colorIdentity: ['G'], ...opts,
  });
}

export function makeSorcery(name: string, cost: string, owner: 0 | 1, opts?: Partial<Card>): Card {
  return createSimpleCard(name, 'Sorcery', cost, owner, {
    colors: [], colorIdentity: [], ...opts,
  });
}

export function putOnBattlefield(state: GameState, card: Card, player: 0 | 1, turn?: number): GameState {
  const entryTurn = turn ?? state.turn;
  const perm = cardToPermanent(card, player, entryTurn);
  const resolved = entryTurn < state.turn ? { ...perm, summoningSick: false } : perm;
  return {
    ...state,
    players: state.players.map((p, i) =>
      i === player ? { ...p, battlefield: [...p.battlefield, resolved] } : p
...
export function passAction(player: 0 | 1): GameAction {
